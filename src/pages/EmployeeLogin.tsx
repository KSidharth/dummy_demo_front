...
const EmployeeLogin: React.FC = () => {
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      ...
      login(response.token, 'employee', response.email);
      navigate('/employee/dashboard', { replace: true });
    } catch (error) {
      const apiError = error as ErrorDto;
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-500 to-teal-600">
      <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-md w-full">
        ...
        <button
          onClick={() => navigate('/')}
          className="w-full mt-4 text-green-600 hover:text-green-800 text-sm font-medium"
        >
          ← Back to Home
        </button>
      </div>
